...
from stable_baselines3.common.torch_layers import BaseFeaturesExtractor, FlattenExtractor, MlpExtractor, NatureCNN, create_mlp, lstm
...
class MlpLstmPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, lr_schedule='constant', n_env = 1, n_batch = 1, activation_fn = tf.tanh, net_arch = None, n_lstm=256, state_shape=(2 * 256, ), reuse=False, 
                 layer_norm=False,
                 **kwargs):
        # state_shape = [n_lstm * 2] dim because of the cell and hidden states of the LSTM
        super(MlpLstmPolicy, self).__init__(sess, ob_space, ac_space, lr_schedule, activation_fn, layer_norm)
        ...
        with tf.compat.v1.variable_scope("model", reuse=reuse):
            latent = tf.compat.v1.layers.flatten(self.preprocessed_obs)
            ...
            for idx, layer in enumerate(net_arch):
                if isinstance(layer, int):  # Check that this is a shared layer
                    ...
                    rnn_output = lstm(input_sequence, masks, self.states_ph, 'lstm1', n_hidden=n_lstm, layer_norm=layer_norm)
                    self.snew = latent = seq_to_batch(rnn_output)
        ...
        if net_arch is None:  # Legacy mode
            ...
            with tf.variable_scope("model", reuse=reuse):
                if feature_extraction == "cnn":
                    extracted_features = cnn_extractor(self.preprocessed_obs, **kwargs)
                else:
                    extracted_features = tf.layers.flatten(self.preprocessed_obs)
